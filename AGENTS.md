...
- **Runtime:** OpenClaw 2026.4.20 on VMware Ubuntu 24.04. The 3 active Telegram-bound agents use `openai/gpt-5.4-mini`; the unused default agent retains `openai/gpt-4o-mini`. `OPENAI_API_KEY` is owned by the OpenClaw Gateway, not this repo.
...
- **9router `gpt-5.5` proxy** at `localhost:20128` was originally configured as primary model but is currently down — the 3 active agents now call `openai/gpt-5.4-mini` directly. Document the failover behavior.
