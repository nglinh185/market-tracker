...
- **2026-04-29** - PR merge + forecast migration applied:
  ...
  - Follow-up run no longer crashed, but Prophet's packaged CmdStan directory on the GitHub runner was invalid (`missing makefile`), so every ASIN warned and the script produced `0 ASINs forecast`. Added GitHub Actions CmdStan cache/install/configure steps (`~/.cmdstan/cmdstan-2.33.1`, `CMDSTAN=$HOME/.cmdstan/cmdstan-2.33.1`) before the ingest analytics step.
  - **Forecast CI fully fixed.** Root cause: Prophet 1.1.5 still checks its bundled path (`site-packages/prophet/stan_model/cmdstan-2.33.1`) even when `CMDSTAN` is configured via cmdstanpy. The workflow now installs CmdStan and symlinks it into Prophet's expected bundled path before running analytics. Verified with `forecast_only`: `success=30 failed=0 skipped_short_history=0`, `210` forecast rows upserted to `price_forecast_daily`.
  - **No-credit reruns added.** `Daily Amazon Data Ingest` manual dispatch now accepts `run_mode`: `full`, `analytics_only`, `forecast_only`. Use `forecast_only` to debug Prophet/Supabase without re-scraping Apify.
  - **Evaluation artifacts generated.** `data/eval/sentiment_eval.json`: `n=3851`, accuracy `0.8117`, macro-F1 `0.5936` (positive F1 `0.908`, negative F1 `0.7147`, neutral F1 `0.158`). `data/eval/forecast_eval.json`: `17` ASINs evaluated, MAE `0.601`, MAPE `1.16%`, naive MAE `0.601`, 80% PI coverage `90.2%`. Forecast eval on Windows skipped 13 ASINs due local Prophet binary crashes; CI Linux forecast run is the canonical production proof (`30/30` ASINs).
  - **Dashboard polish committed.** Added `.streamlit/config.toml`, `dashboard/utils/theme.py`, sidebar branding, Plotly theme defaults, and Sentiment page filtering to ASINs that actually have review data.
  - **Telegram voice hardening committed.** All 3 SOUL.md files now explicitly ban offer-to-dive-deeper closers (`If you want...`, `Nếu muốn...`); final line must be an action, watch item, or caveat.
