...
All fourteen OpenClaw skills are read-only command-line Python modules. They follow the same contract documented in \texttt{openclaw/AGENTS.md}: a module-level \texttt{SKILL} dictionary, an importable \texttt{run(**kwargs)} function, JSON arguments from the command line, JSON output to stdout, and a \texttt{--schema} option.
