...
Two baselines were included to contextualise the result: VADER \citep{hutto2014vader}, a rule-based lexicon analyzer, and \seqsplit{nlptown/bert-base-multilingual-uncased-sentiment}, a BERT model fine-tuned directly on 1--5 star Amazon and Yelp reviews. The VADER compound score was thresholded at $\pm 0.05$ following the authors' convention. Table~\ref{tab:sentiment_baselines} reports the three-way comparison.
...
\begin{figure}[H]
  ...
  \caption{Average RoBERTa sentiment composition by product category in the latest analysed review snapshot. Per-category sample sizes are unequal: Gaming Keyboards and True Wireless Earbuds each aggregate sentiment over $n = 10$ watchlist ASINs, while Portable Chargers covers only $n = 5$ ASINs. The Portable Charger bar should therefore be interpreted as indicative rather than representative; a single low-sentiment ASIN moves the category mean by roughly twice as much as in the other two categories.}
  \label{fig:sentiment_dist}
\end{figure}

\subsubsection*{Confusion matrix}

Table~\ref{tab:roberta_confusion} shows the full confusion matrix for the RoBERTa
classifier on the $N = 3{,}851$ proxy-labeled reviews.

\begin{table}[H]
  \centering
  \begin{tabular}{lrrr}
    \toprule
    \textbf{True \textbackslash\ Predicted} & \textbf{Negative} & \textbf{Neutral} & \textbf{Positive} \\
    \midrule
    Negative (698)   & \textbf{546} & 76  & 76 \\
    Neutral (299)    & 136          & \textbf{47} & 116 \\
    Positive (2{,}854) & 148        & 173 & \textbf{2{,}533} \\
    \bottomrule
  \end{tabular}
  \caption{Confusion matrix for RoBERTa on $N = 3{,}851$ reviews with star-rating proxy labels.
  Diagonal entries (bold) are correct predictions. The neutral class has the lowest recall (15.7\%),
  reflecting the inherent ambiguity of 3-star reviews. Positive-as-neutral misclassifications (173)
  account for the main source of positive-class error.}
  \label{tab:roberta_confusion}
\end{table}

The matrix reveals asymmetric error patterns. Negative reviews are relatively well-recovered
($\text{recall} = 78.2\%$), with misclassifications split evenly between neutral and positive.
Neutral recall is the weakest (15.7\%): the model most often assigns neutral reviews to either
the negative or positive class, reflecting the conceptual ambiguity of 3-star text which can
contain mixed positive and negative statements. The positive class dominates the dataset (74\% of
reviews) and achieves the highest $F_1$. For the operational use case --- tracking whether
customer opinion is shifting rather than counting exact label proportions --- the key concern is
negative detection, where 78.2\% recall is acceptable for trend monitoring.

\subsubsection*{Manual label validation}

To assess whether the star-rating proxy systematically under- or over-estimates
RoBERTa's true performance, 200 reviews were manually labeled by the author and
evaluated against both the star-proxy labels and the RoBERTa predictions.
Table~\ref{tab:manual_label_eval} reports the three-way accuracy comparison.

\begin{table}[H]
  \centering
  \begin{tabular}{lcc}
    \toprule
    \textbf{Comparison} & \textbf{Accuracy} & \textbf{Interpretation} \\
    \midrule
    Star proxy vs.\ manual labels  & 0.745 & Star ratings carry 25.5\% label noise \\
    RoBERTa vs.\ manual labels     & \textbf{0.720} & True model accuracy \\
    RoBERTa vs.\ star proxy        & 0.635 & Apparent accuracy under proxy evaluation \\
    \bottomrule
  \end{tabular}
  \caption{Three-way accuracy comparison on 200 manually labeled Amazon reviews.
  All rows use the same sample of 200 reviews labeled by the author.
  ``Star proxy vs.\ manual'' measures proxy label noise.
  ``RoBERTa vs.\ manual'' is the primary measure of true model quality.
  ``RoBERTa vs.\ star proxy'' replicates the proxy evaluation on the same
  subset for direct comparability.}
  \label{tab:manual_label_eval}
\end{table}

The results reveal that star ratings agree with human judgment on only 74.5\% of
reviews, confirming that the proxy carries meaningful label noise, concentrated
in ambiguous 3-star reviews and cases where buyers rate highly despite expressing
concerns in the text body. Crucially, RoBERTa's true accuracy against manual
labels (72.0\%) is 8.5 percentage points higher than its apparent accuracy under
the star-proxy evaluation (63.5\%). This gap indicates that a substantial fraction
of the model's apparent ``errors'' are in fact cases where RoBERTa correctly
identifies the sentiment of the text and the star rating is the noisy signal.
The star-proxy evaluation therefore \emph{underestimates} RoBERTa's genuine
classification ability. A 72.0\% three-class accuracy on product reviews is
consistent with published benchmarks for this model family on Amazon review data
\citep{barbieri2020tweeteval} and is sufficient for the trend-detection and
comparative-ranking use cases that motivate the deployment.
...
\begin{enumerate}
  ...
  \item \textbf{Prophet}: Facebook Prophet with the \texttt{CMDSTANPY} backend, with daily, weekly, and yearly seasonality disabled. Prophet is retained in the deployed system because it additionally produces native 80\% prediction intervals (\texttt{yhat\_lower}, \texttt{yhat\_upper}), which the Momentum Strategist agent exposes as volatility signals.
\end{enumerate}
...
An important caveat is that 22 of the 30 watchlist ASINs (73\%) exhibited no price change across all three holdout days, so their held-out error was identically zero for every model. The pooled MAE figures are therefore disproportionately influenced by the remaining 8 ASINs with non-zero price movement; the most volatile ASIN in this subset contributed a naive holdout MAE of \$20.00 to the pooled average. This distribution should be kept in mind when interpreting the pooled figures: the comparison primarily captures model behaviour on a small number of actively-priced products rather than the watchlist as a whole.

\subsubsection*{Interpretation}

The dominance of the naive baseline across all more complex alternatives is a well-documented property of short, near-stationary price series \citep{makridakis2018m4}. Amazon marketplace prices are frequently held constant for days or weeks; in such conditions, carry-forward trivially minimises expected absolute error, and any model that attempts to learn trend or autocorrelation risks introducing estimation noise that exceeds its signal gain. The near-equivalence of ARIMA and naive (pooled MAE gap of \$0.031) is in fact stronger than the table suggests: ARIMA(1,1,0) produced predictions identical to naive carry-forward on 29 of the 30 watchlist ASINs, with the single exception being B0CDX5XGLK (where ARIMA differed by \$0.940 on a flat-priced holdout). The entire pooled gap is therefore attributable to one ASIN, and the differenced AR(1) structure captures essentially no additional predictable autocorrelation beyond what carry-forward already exploits.

The underperformance of ETS, Trend, and Prophet relative to naive is consistent with overfitting on training windows of 13--15 observations: each model has sufficient parameters to learn the specific trajectory of the training prices rather than the underlying generating process. This is an inherent data-volume constraint rather than a deficiency in the models themselves.

\subsubsection*{Model selection for deployment}

Despite ranking fifth on point-forecast MAE, Prophet is retained as the production forecasting model for two reasons. First, it is the only model whose 80\% prediction intervals were extracted, persisted to Supabase, and coverage-tested in this evaluation pipeline (\texttt{yhat\_lower}, \texttt{yhat\_upper}); the Momentum Strategist agent uses these intervals to signal short-term price volatility rather than a precise point estimate. The empirical coverage was 67.8\%, falling below the nominal 80\% target and indicating that the intervals are currently undercalibrated --- a known consequence of limited training data. While the \texttt{statsmodels} ARIMA implementation can in principle produce confidence intervals via \texttt{get\_forecast()}, the deployed pipeline did not extract or evaluate them, so no calibrated uncertainty envelope is available from ARIMA or naive in the current system. Second, Prophet's accuracy disadvantage is expected to diminish as data accumulates: with 30 or more observations per ASIN, the model's changepoint detection and trend-fitting become more reliable. The thesis system is designed as a continuous pipeline, so forecast quality should improve organically over time without requiring code changes.
...
\subsubsection*{Top-BMS and top/bottom-LQS ASINs}

Tables~\ref{tab:bms_top5} and~\ref{tab:lqs_extremes} present the five highest-BMS
watchlist ASINs and the LQS extremes from the latest analytics snapshot, providing
concrete examples of the scores described above.
...
\begin{table}[H]
  ...
  \begin{tabular}{llcccrc}
    \toprule
    \textbf{Rank} & \textbf{ASIN} & \textbf{Category} & \textbf{BMS} & \textbf{BSR Vel.} & \textbf{Rev.\ Vel.} & \textbf{Sent.} \\
    \midrule
    1 & B0CB1FW5FC  & Portable Charger & 0.752 & $+0.125$ & 116 & $+0.710$ \\
    2 & B0CRTR3PMF  & TWE              & 0.739 & $+0.015$ & 142 & $+0.849$ \\
    3 & B09DT48V16  & TWE              & 0.735 & $+0.087$ &  59 & $+0.629$ \\
    4 & B0DGHMNQ5Z  & TWE              & 0.715 & ---      &  73 & $+0.654$ \\
    5 & B0DD41G2NZ  & TWE              & 0.710 & $+0.186$ &  42 & $+0.613$ \\
    \bottomrule
  \end{tabular}
  \caption{Top-5 watchlist ASINs by Brand Momentum Score from the 2026-04-29 snapshot. Columns show the raw inputs to the BMS formula defined in \S3.3.5 (BSR velocity as fractional rank improvement; review velocity in new reviews per day, capped at 50 during normalisation; sentiment scalar in $[-1,1]$). ``---'' indicates BSR velocity unavailable for that ASIN, in which case the implementation defaults the normalised BSR signal to 0.5. TWE = True Wireless Earbuds.}
  \label{tab:bms_top5}
\end{table}
...
\begin{table}[H]
  ...
  \begin{tabular}{llcc}
    \toprule
    \textbf{Group} & \textbf{ASIN} & \textbf{Category} & \textbf{LQS} \\
    \midrule
    \multirow{5}{*}{Top 5}
      & B0C9ZJHQHM & Gaming Keyboard  & 96.9 \\
      & B0CRTR3PMF & TWE              & 96.8 \\
      & B0CDX5XGLK & Gaming Keyboard  & 96.7 \\
      & B0CLLHSWRL & Gaming Keyboard  & 96.7 \\
      & B0CB1FW5FC & Portable Charger & 96.6 \\
    \midrule
    \multirow{5}{*}{Bottom 5}
      & B0FQFB8FMG & TWE              & 79.5 \\
      & B0DGHMNQ5Z & TWE              & 81.7 \\
      & B0G1PJLWLZ & TWE              & 88.3 \\
      & B0DCZ56QNL & Portable Charger & 90.2 \\
      & B0CFHHCMNS & Portable Charger & 91.4 \\
    \bottomrule
  \end{tabular}
  \caption{Top-5 and bottom-5 watchlist ASINs by Listing Quality Score (LQS) from the 2026-04-29 snapshot. LQS is scored from 0--100 across seven active components: title length, bullet count, image presence, A+ content, star rating, review count, and sentiment. The Bottom-5 group is ordered from lowest to highest LQS. TWE = True Wireless Earbuds.}
  \label{tab:lqs_extremes}
\end{table}

The BMS table is dominated by True Wireless Earbuds (4 of 5), with Portable Charger B0CB1FW5FC as the top mover. B0CB1FW5FC and B0CRTR3PMF both saturated the review-velocity ceiling (raw 116 and 142 new reviews respectively, capped at 50 in normalisation), which is the second-largest weight in the BMS formula after BSR velocity; B0CRTR3PMF additionally has the highest sentiment of the group ($+0.849$). The LQS extremes table illustrates that even the bottom-scoring ASIN (B0FQFB8FMG, 79.5) is not a weak listing by absolute standards: four of the bottom five score above 88, and the top-five spread is only 0.3 points (96.6--96.9), consistent with the high-floor observation reported earlier.

\subsubsection*{Validation attempt and limitations}

An attempt was made to validate LQS internal consistency via Cronbach's alpha
and BMS forward predictive validity via Spearman rank correlation
($\rho$, lag = 5 days). Both analyses returned uninformative results due to
data structure limitations rather than model design errors.

For LQS, three of seven components --- title score, bullet score, and image
score --- exhibited zero variance across all 540 watchlist-ASIN-day
observations: every tracked ASIN had a title longer than ten words, five or
more bullet points, and a main product image. The ceiling effect is a direct
consequence of curating a watchlist from established top-ranked products. With
three constant items, Cronbach's alpha is mathematically undefined and the
result ($\alpha = -0.13$) is artefactual. LQS cannot be validated by internal
consistency on this dataset and should be interpreted as a formative composite
whose components are independent dimensions of listing quality, not as a
reflective scale.

For BMS, per-ASIN Spearman $\rho$ between BMS$_t$ and BSR$_{t+5}$ had a
median of $+0.13$ with 79\% of ASINs showing a positive (wrong-sign)
correlation. The pooled negative correlation ($\rho = -0.46$) reflects a
cross-sectional confound: ASINs that are persistently high-quality tend to have
both high BMS and low BSR regardless of day-to-day movement. BMS does not
exhibit temporal forward predictive validity within individual ASIN trajectories
over the observation window, confirming its intended role as a
descriptive ranking heuristic rather than a predictive momentum model.
...
The same Supabase-derived tables supported both pull-based and push-based consumption. The Streamlit dashboard and public web dashboard provide analyst-controlled exploration, while the OpenClaw agents consume the data through 14 read-only Python skills. This separation is important: analytics scripts and scheduled jobs write to Supabase, but agent skills only read and summarize existing records. The resulting architecture reduces the risk of accidental database mutation through conversational interfaces.

The three Telegram-bound agents also map cleanly to the analytical outputs. Competitor Spy uses ranking, BMS, entrant/exit, sponsored-share, price-tier, and image-change skills. Sentiment Detective uses sentiment, review, aspect, and snapshot skills. Momentum Strategist uses alerts, BMS, price forecast, LQS, sentiment, and listing-content skills. The pilot therefore demonstrates that the same analytics backbone can support multiple specialized decision views without duplicating data pipelines.
