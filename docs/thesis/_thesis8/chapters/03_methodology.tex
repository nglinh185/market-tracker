...
\texttt{gpt-5.4-mini}~\citep{openai2026gpt54mini} inference calls routed
...
\begin{table}[H]
  ...
  \begin{tabular}{lll}
    ...
    OpenAI \texttt{gpt-5.4-mini}~\citep{openai2026gpt54mini} & Pay-as-you-go (\$0.75/MTok input) & $<$ \$1 \\
    \midrule
...
\end{align}
The denominator of 50 in the review-velocity normalisation is a chosen
ceiling: any ASIN gaining 50 or more new reviews in a single day receives
the maximum normalised review score of 1.0. This threshold was set
empirically to separate routine daily review accumulation from a viral
or promotionally-driven review surge.
...
registered tools. The three specialist agents route their inference
through OpenAI's \texttt{gpt-5.4-mini} model
\citep{openai2026gpt54mini}, which is OpenAI's officially released
mini-tier model designed specifically for coding, tool use, and
subagent workflows. The model supports a 400,000-token context window
and up to 128,000 output tokens, and exposes structured function
calling, which the OpenClaw Gateway uses to invoke the registered
Python skills \citep{openai2026gpt54mini}. It was selected over the
full \texttt{gpt-5.4} model because of its lower per-token cost
(\$0.75 per million input tokens) and lower inference latency, both
of which are important for a high-frequency scheduled-brief workload.
The deployment uses one-to-one channel binding: each
...
\begin{enumerate}
    ...
    Uses six skills: \texttt{query\_alerts}, \texttt{query\_bms},
    \texttt{query\_price\_forecast}, \texttt{query\_lqs},
    \texttt{query\_sentiment}, and \texttt{query\_listing\_content}.
\end{enumerate}

\subsection{Skill executions (tools)}

The project registers 14 read-only Python skills under
...
The primary evaluation uses star ratings as noisy labels for the full $N = 3{,}851$ review corpus, as manually labelling a dataset of this scale was not feasible within the thesis timeline:
\begin{equation}
  \label{eq:star_proxy}
...
\subsection{Sentiment classification: manual label validation}
\label{subsec:manual_eval}

To quantify the noise introduced by the star-rating proxy described above, a stratified
sample of 200 reviews was manually labeled by the author. Reviews were sampled
proportionally from all three product categories and assigned a ground-truth label
(\textit{negative}, \textit{neutral}, or \textit{positive}) based solely on review
text, without reference to the associated star rating or the RoBERTa prediction.
Manual labeling was performed in a single session to minimise intra-rater drift.

The labeled sample enables three targeted comparisons:

\begin{enumerate}
    \item \textbf{Star-proxy accuracy against manual labels:} measures how often the
    star-rating mapping (Equation~\ref{eq:star_proxy}) agrees with human judgment,
    thereby quantifying the ceiling on any evaluation that uses stars as ground truth.

    \item \textbf{RoBERTa accuracy against manual labels:} measures the model's genuine
    classification performance on text, independent of star-rating noise.
    This is the primary measure of true model quality.

    \item \textbf{RoBERTa accuracy against star-proxy labels (same 200-sample):}
    replicates the proxy evaluation on the same subset to make the three comparisons
    directly comparable.
\end{enumerate}

The gap between comparisons (2) and (3) directly quantifies how much of RoBERTa's
apparent error under proxy evaluation is attributable to noisy labels rather than
genuine misclassification. This validation is implemented in
\texttt{scripts/evaluate\_sentiment\_manual.py} and produces
\seqsplit{data/eval/sentiment\_manual\_eval.json}.
...
least $\texttt{MIN\_HISTORY} + H = 10$ price observations, the final
$H = 3$ days are held out, each candidate model is fitted on the earlier
observations, and predictions are compared with actual holdout prices.
For each ASIN and model:
\begin{align}
  \text{MAE}_i  &= \frac{1}{H}\sum_{t=1}^{H} |y_{i,t} - \hat{y}_{i,t}|, \\
  \text{MAPE}_i &= \frac{1}{H}\sum_{t=1}^{H}
  \frac{|y_{i,t} - \hat{y}_{i,t}|}{y_{i,t}} \times 100\%.
\end{align}
Pooled MAE and MAPE are then computed by concatenating held-out points
across all evaluated ASINs. Five methods are compared: (1) a naive
carry-forward baseline, (2) a linear trend (OLS) baseline, (3)
Exponential Smoothing with additive trend (ETS/Holt), (4) ARIMA(1,1,0),
and (5) Prophet. For ARIMA and ETS, convergence failures on individual
ASINs (expected with $n \approx 7$--$12$ flat series) trigger an
automatic fallback to the naive prediction for that ASIN. The script
additionally reports Prophet's 80\% prediction-interval coverage to
assess whether the uncertainty envelope is well-calibrated.
...
\begin{table}[htbp]
  ...
  \begin{tabular}{p{3.4cm}p{2.6cm}p{7.0cm}}
    ...
    Forecast eval & 3-day holdout, min 10 obs., 5 models & Naive, OLS, ETS, ARIMA(1,1,0), Prophet compared in \texttt{evaluate\_forecast.py}. \\
